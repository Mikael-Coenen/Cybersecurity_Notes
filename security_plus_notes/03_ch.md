...
- Significant portion of ransomware attacks are from phishing campaigns, some are from direct attack methods
- IOC's include:
...
- Goal is for attackers to capture user input and analyze it
...
- IOC's include:
 ...
 ## Logic bombs
 - Malware that could be functions or code placed inside programs that will activate when set conditions are met
 - Logic bombs are not independent malicious programs (unlike the rest of the types of malware in this chapter)
 - IOC's are less common -- they require analysis of code or logic in the application
 - Mitigation practice: code review

## Rootkits
- Malware thats designed to allow attackers to access a system through a backdoor
- The best way to detect a rootkit is to test the suspected device from a trusted system
- IOC's include:
    - Files hashes and signatures
    - C&C domains, IP addresses, and systems
    - Behavior-based identification like the creation of executables, files access, etc
